...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999, lambda_=0.99):
        ...
        for t in range(len(self.rewards) - 1, -1, -1):
            ...
            R = r + gamma * R
            returns.insert(0, R)
            state_value.append(saved_actions[t][1])
        ...
        # list of estimated advantage in each time step
        gae_estimator = GAE(gamma=gamma, lambda_=lambda_, num_steps=len(self.rewards))
        est_adv_list = gae_estimator(self.rewards, state_value)

        # calculate policy loss using GAE
        for step in range(len(saved_actions)):
            log_prob = saved_actions[step][0]
            est_adv = est_adv_list[step]
            policy_losses.append(-est_adv * log_prob)
...
def train(lr=0.01):
    ...
    for i_episode in count(1):
        ...
        loss = model.calculate_loss(gamma=0.999, lambda_=0.95)
        ...
        if ewma_reward > 120:
            if not os.path.isdir("./preTrained"):
                os.mkdir("./preTrained")
            torch.save(model.state_dict(), './preTrained/LunarLander_{}_GAE.pth'.format(lr))
...
if __name__ == '__main__':
    ...
    train(lr)
    test(f'LunarLander_{lr}_GAE.pth')
